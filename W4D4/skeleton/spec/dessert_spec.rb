...
describe Dessert do
  ...
  let(:cookie) {Dessert.new("chocolate chip", 100, chef)}
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("pie", "100", chef) }.to raise_error(ArgumentError)
    end
  end

  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do
      cookie.add_ingredient("brown sugar")
      expect(cookie.ingredients).to include("brown sugar")
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      ...
      expect(cookie.serve).to eq("Chef chef the Great Baker has made 100 chocolate chips!")
    end
  end
